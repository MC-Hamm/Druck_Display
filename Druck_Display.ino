...
#define ANALOG_SOCKET_VALUE 70 // Nullpunkt einstellen, Sensor liefert bei 0 Druck 0,5 Volt 
...
byte SENSOR_EN_PINS[]      {5}; //VCC Pin des Sensors(rot) - Sensor wird zur Messung an Spannung gelegt (wird aktuell nicht benutzt, Ausgang Spannung messen)
byte SENSOR_PINS[]         {14}; //Eingang Pin des Arduinos - Analog Ausgang des Sensors(gelb)
...
class WeatherChannel : public Channel<Hal, UList1, EmptyList, List4, PEERS_PER_CHANNEL, UList0>, public Alarm {
  ...
  public:
    ...
    void measure() {
      digitalWrite(SENSOR_EN_PINS[(number() - 1)], HIGH);       // Sensor(rot) wird eingeschaltet 
      _delay_ms(500);
      uint16_t sens_val = 0;
      for (uint8_t i = 0; i < 10; i++) {                        // es werden 10 Messungen gemacht und die Werte addiert
        sens_val += analogRead(SENSOR_PINS[(number() - 1)]); 
        _delay_ms(5);
      }
      sens_val = sens_val / 10;                                 // ...und wieder durch 10 geteilt = Messwerte glätten
      digitalWrite(SENSOR_EN_PINS[(number() - 1)], LOW);        // ADC Pin(Sensor) wird wieder auf LOW geschaltet 
      ...
      float _p = (((sens_val / 1024.0) - (ANALOG_SOCKET_VALUE / 1000.0)) / sensor_factor) * 1000; // Fließkomma Wert in KPa berechnen
      pressure = _p > 0 ? _p : 0;
      ...
      String vStr = " " + String(_p / 100) + " Bar"; //OLED Anzeige in Bar - von KPa nach Bar faktor 100
      displayLine1 = vStr; // String wird zur Darstellung übergeben
      printDisplay(); // Unterprogramm wird zur Darstellung aufgerufen
    }

    void printDisplay() {
      for (int i = displayLine1.length(); i < 6; i++) {  // Zeile auffüllen
      displayLine1 += " ";
      }
